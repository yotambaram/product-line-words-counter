...
async function getList(path) {
  ...
  const matchingResults = await resultMatching(trieRoot2, titleArr);



  let ThirdCleanDataArr = [];
  for (let i = 0; i < matchingResults.length; i++) {
    // const element = matchingResults[i].line;
    if (
      // Has line
      matchingResults[i].line &&
      matchingResults[i].line != "### No Title ###" &&
      matchingResults[i].line != "root" &&
      // has brand
      matchingResults[i].brand
    ) {
      let line = _.split(matchingResults[i].line, " ")
      line.unshift(matchingResults[i].brand.toLowerCase())
      ThirdCleanDataArr.push(line)
    }
  }
  
  const trieRoot3 = trieBuilder(ThirdCleanDataArr, oneWordStatsTree);
  const trieResultsArr2 = trieRoot3.getLines();

  // for (let i = 0; i < trieResultsArr2.length; i++) {
  //   // const element = matchingResults[i].line;
  //   if (
  //     // Has line
  //     matchingResults[i].line &&
  ...
  /////////////////
  const csvFromArrayOfArrays2 = convertArrayToCSV(trieResultsArr2, {
    // header,
    separator: ",",
  });
  ...
  // print
  const csvFromArrayOfArrays = convertArrayToCSV(ThirdCleanDataArr, {
    // header,
    separator: ",",
  });
